...
webinputs = [("Launch Site",(("Launch Time",("Hours Until Launch", '')),("Latitude",("Latitude","(Decimal Degrees)")),("Longitude",("Longitude","(Decimal Degrees)")),
             ("Launch Altitude",("Launch Altitude","(feet)")))),("Flight Properties",(("Burst Altitude",("Burst Altitude","(feet)")),
...
def webPredict(data):
    ...
    "Ascent Rate":("Ascent Rate: ","(feet/minute)"),"Descent Rate":("Descent Rate: ","(feet/minute)")}"""
    keys = {"Launch Time":int,"Latitude":float,"Longitude":float,"Launch Altitude":int,"Burst Altitude":int,
            "Ascent Rate":int,"Descent Rate":int}
    for key in keys:
        try:
            keys[key](data[key])
    ...
    try:
        wxstation = getClosestStation((float(data["Latitude"]),float(data["Longitude"])))
        winddata = getData(wxstation=wxstation[0],hourdifference=int(data["Launch Time"]))
        atmosphere=parse_wind(winddata)
        track,trackdown=makeTrack(atmosphere,(float(data["Latitude"]),float(data["Longitude"]),int(data["Launch Altitude"])),
                        int(data["Ascent Rate"]),int(data["Descent Rate"]),int(data["Burst Altitude"]),criticalpoints=True)
        #raise PredictionException, trackdown
        return track, trackdown
    except:
        raise PredictionError, str(traceback.format_exc())

def main(launchtime=None,launchsite=(40.191903,-110.38099,5826),ascentrate=1045,descentrate=1763,burstaltitude=98000,
...
if __name__=="__main__":
    ...
    launchsite = (39.6676,-113.1269,5220)
